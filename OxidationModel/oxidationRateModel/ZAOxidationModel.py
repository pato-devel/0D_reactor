...
class ZAOxidationModel(Ox.OxidationRateModelSelector, ABC):
    ...
    B = 3.5E19 # surface site concentration [1/m2]
    ...
    def compute_rates(self, k, w_Os):
        ...
        delta_gibbs_O2 = 2*Gibbs_energy[self.mixture.speciesIndex('O')] -  Gibbs_energy[self.mixture.speciesIndex('O2')]
        
        over_K_one = self.B*self.k_B*self.T_beam/Po*math.pow(2*math.pi*self.m_O*self.k_B*self.T_beam/self.h**2,0.5)*math.exp(-45000/self.T_beam)
        K_one = 1./over_K_one
        
        Kc_O2 = math.exp(-delta_gibbs_O2/(self.R*self.T_beam))
        
        Kp_O2 = math.pow(Po/(self.R*self.T_beam),1)*Kc_O2
        
        K3 = over_K_one*over_K_one/Kp_O2 # Equilibrium constant
