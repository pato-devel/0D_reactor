...
class PrataOxidationModel(Ox.OxidationRateModelSelector, ABC):
    ...
    def compute_rates(self, Tw):
        ...
        F_O_2D = math.sqrt(math.pi * self.k_B * Tw / (
                2 * self.m_O))  # mean thermal speed of the mobile adsorbed O-atom on the surface [m/s]

        # reaction rates
        k = {}
        k["kO1"] = self.F_O * 0.3 / self.B
        k["kO2"] = 2 * math.pi * self.m_O * self.k_B ** 2 * Tw ** 2 / (self.Av * self.B * self.h ** 3) * math.exp(
            -44277 / Tw)
        k["kO3"] = self.F_O / self.B * 100 * math.exp(-4000 / Tw)
        k["kO4"] = self.F_O / self.B * math.exp(-500 / Tw)
        k["kO5"] = self.F_O / self.B * 0.7
        k["kO6"] = 2 * math.pi * self.m_O * self.k_B ** 2 * Tw ** 2 / (self.Av * self.B * self.h ** 3) * math.exp(
            -96500 / Tw)
    ...
    def fun_f_CO(self, Tw, t, k, w_s, w_Os, w_Oss):
        ...
        """
        f_CO = k["kO3"] * self.w_O * w_Os + k["kO7"] * self.w_O * w_Oss + k["kox2"] * self.w_O2 * w_Os + k[
            "kox5"] * self.w_O2 * w_Oss
        return f_CO
    ...
    def fun_f_O(self, Tw, t, k, w_s, w_Os, w_Oss):
        ...
        f_O = press_O / (self.Av * math.sqrt(2 * math.pi * self.m_O * self.k_B * self.T_beam)) \
              - k["kO1"] * self.w_O * w_s + k["kO2"] * w_Os - k["kO4"] * self.w_O * w_Os - k["kO5"] * self.w_O * w_s + \
              k["kO6"] * w_Oss \
